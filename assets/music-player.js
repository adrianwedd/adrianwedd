class RetroMusicPlayer {
  ...
  async playTrack(trackName) {
    ...
    if (this.tracks[trackName]) {
      ...
      console.error(
        `Track "${trackName}" not found in available tracks:`,
  ...
  createSynthwaveTrack() {
    const melody = [261.63, 293.66, 329.63, 392.0, 329.63, 293.66]; // C4, D4, E4, G4, E4, D4
  ...
  createProceduralTrack() {
    ...
    const { oscillator: droneOsc, gainNode: droneGain } = this.createOscillator(
      droneFreq,
      'sawtooth'
    );
