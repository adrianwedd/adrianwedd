class AIService {
  ...
  shouldCachePrompt(prompt, systemPrompt = '') {
    ...
    return (
      estimatedTokens >= this.cacheConfig.minTokensForCache ||
  ...
  async sendChatRequest(userMessage, sessionId, systemContext = null) {
    ...
    try {
      ...
      const estimatedInputTokens = this.estimateTokenCount(JSON.stringify(requestBody.messages));
  ...
  async makeAPIRequest(requestBody, sessionId) {
    ...
    const endpoints = ['/api/chat-anthropic', '/api/ai/chat', 'https://your-domain.com/api/ai'];
  ...
  generateMockResponse(requestBody) {
    ...
    return {
      response:
        'This is a mock AI response demonstrating token counting and caching. ' +
  ...
  getTokenStats() {
    ...
    const cacheEfficiency =
      this.tokenStats.totalRequests > 0
        ? (this.tokenStats.cacheHits / this.tokenStats.totalRequests) * 100
        : 0;
