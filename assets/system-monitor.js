class SystemMonitor {
  ...
  async updateAllData() {
    ...
    try {
      await Promise.all([this.updateCIData(), this.updateHomeData(), this.updateAIData()]);
    } catch (error) {
  ...
  async updateCIData() {
    try {
      ...
      const response = await fetch(
        'https://api.github.com/repos/adrianwedd/adrianwedd/actions/runs?per_page=10',
        {
          headers: {
  ...
  generateMockCIData() {
    ...
    this.ciData = workflows.map((name, i) => {
      ...
      const conclusion =
        status === 'completed' ? (Math.random() > 0.8 ? 'failure' : 'success') : null;
  ...
  renderAIData(stats) {
    ...
    const tokenSavings =
      stats.cachedTokens > 0
        ? ((stats.cachedTokens / (stats.totalTokens + stats.cachedTokens)) * 100).toFixed(1)
        : '0.0';
  ...
  processTokenAnalytics(data) {
    ...
    const todaySessions = data.session_usage.filter(
      (s) => s.utc_timestamp && s.utc_timestamp.startsWith(today)
    );
    ...
    const avgCacheRatio =
      todaySessions.length > 0
        ? todaySessions.reduce((sum, s) => sum + (s.cache_hit_ratio || 0), 0) / todaySessions.length
        : 0;
    ...
    return {
      ...
      lastSession:
        todaySessions.length > 0 ? todaySessions[todaySessions.length - 1].utc_timestamp : null,
  ...
  updateSystemStats() {
    ...
    this.systemStats.memory.used = Math.max(
      0.8,
      Math.min(15, this.systemStats.memory.used + (Math.random() - 0.5) * 0.2)
    );
    this.systemStats.network.down = Math.max(
      0.1,
      this.systemStats.network.down + (Math.random() - 0.5) * 0.5
    );
    this.systemStats.network.up = Math.max(
      0.05,
      this.systemStats.network.up + (Math.random() - 0.5) * 0.2
    );
    ...
    document.getElementById('memUsage').textContent =
      `${this.systemStats.memory.used.toFixed(1)}GB/${this.systemStats.memory.total}GB`;
    document.getElementById('netActivity').textContent =
      `▼${this.systemStats.network.down.toFixed(1)}MB ▲${this.systemStats.network.up.toFixed(1)}MB`;
  }
