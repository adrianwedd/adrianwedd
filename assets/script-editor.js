...
echo \${message}"></textarea>
    ...
    if (e.key === 'Tab') {
      ...
      if (e.shiftKey) {
        ...
        if (lineText.startsWith('    ')) {
          this.textArea.value =
            value.substring(0, lineStart) + lineText.substring(4) + value.substring(end);
      ...
      if (indentMatch) {
        setTimeout(() => {
          ...
          this.textArea.selectionStart = this.textArea.selectionEnd =
            currentStart + indentMatch[1].length;
          this.updateLineNumbers();
          this.updateStatusBar();
        }, 0);
      }
  ...
  openEditor(scriptName = null) {
    ...
    this.terminal.addOutput(
      '📝 Script editor opened. Use Ctrl+S to save, Ctrl+R to run, Esc to close.',
      'info'
    );
  }
  ...
  saveCurrentScript() {
    ...
    if (this.currentScript) {
      ...
      this.currentScript = this.scriptEngine.createScript(
        scriptName,
        content,
        'User created script'
      );
