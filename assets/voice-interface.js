class VoiceInterface {
  ...
  generateFallbackResponse(transcript) {
    const responses = {
      ...
      projects:
        "I'm currently working on TicketSmith, this terminal interface, and VERITAS AI safety research.",
      skills:
        'My expertise includes AI systems, recursive architecture, and sustainable computing on my off-grid homestead.',
      homestead:
        'I operate from a 170-acre off-grid homestead in Tasmania with solar power and satellite internet.',
    ...
    utterance.onstart = () => {
      ...
      if (window.accessibilityManager) {
        window.accessibilityManager.announce(
          `Speaking: ${cleanText.substring(0, 50)}...`,
          'polite'
        );
      }
    };
  ...
  updateVoiceOutputIndicator(enabled) {
    const statusText = document.getElementById('voiceStatus');
    if (statusText) {
      const baseText = statusText.textContent
        .replace(' | Speech: ON', '')
        .replace(' | Speech: OFF', '');
