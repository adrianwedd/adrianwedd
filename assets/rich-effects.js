class RichEffects {
  ...
  createTable(headers, rows, style = 'default') {
    ...
    const headerRow =
      s.border +
      headers.map((header, i) => ` ${header.padEnd(colWidths[i] - 1)}`).join(s.border) +
      s.border;
    this.terminal.addOutput(headerRow, 'command');

    // Separator
    const separator =
      s.corner + colWidths.map((width) => s.horizontal.repeat(width)).join(s.corner) + s.corner;
    ...
    rows.forEach((row) => {
      const dataRow =
        s.border +
        row.map((cell, i) => ` ${String(cell || '').padEnd(colWidths[i] - 1)}`).join(s.border) +
        s.border;
