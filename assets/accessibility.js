...
class AccessibilityManager {
  ...
  addMissingHeadings() {
    ...
    const paneTitles = document.querySelectorAll(
      '.pane-title:not(h1):not(h2):not(h3):not(h4):not(h5):not(h6)'
    );
          ...
          if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
            mutation.addedNodes.forEach((node) => {
              if (
                ...
                (node.classList.contains('output-line') || node.classList.contains('command-line'))
              ) {
  ...
  enhanceElement(element) {
    ...
    const progressBars = element.querySelectorAll
      ? element.querySelectorAll('.progress-bar:not([role])')
      : [];
