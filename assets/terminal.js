...
-rwxr-xr-x  adrian adrian  8192 Jul 24 11:45 ai-persona*
  ...
  continueBootSequence() {
    this.currentBootMessages = [
      ...
      'Password: ********',
      '',
      'Welcome to ADRIAN.OS Terminal Interface',
  ...
  async fetchGitHubWorkflowStatus() {
    ...
    try {
      ...
      const response = await fetch(
        'https://api.github.com/repos/adrianwedd/adrianwedd/actions/runs?per_page=5',
        {
          headers: {
      ...
      const statusEmoji =
        githubStatus.status === 'success'
          ...
          : githubStatus.status === 'failure'
            ? '❌'
            : githubStatus.status === 'in_progress'
              ? '🔄'
              : '⏸️';

      messages.push(
        `${statusEmoji} Status: ${githubStatus.status} (Run #${githubStatus.recentRun.runNumber})`
      );
      messages.push(
        `📊 Success rate: ${githubStatus.summary.successRate}% (${githubStatus.summary.passed}/${githubStatus.summary.total})`
      );
...
░░░       ░░░░░   ░░░░░ ░░░░░░░░░░   ░░░░░   ░░░░░ ░░░░░ ░░░░░   ░░░░░ ░░░░░    ░░░░░`;
  ...
  showCIHelp() {
    ...
    this.addOutput('', 'info');
    this.addOutput(
      '<span class="command-name">ci status</span>                     - Show current CI/CD status',
      'feature-highlight',
      true
    );
    this.addOutput(
      '<span class="command-name">ci logs</span> [latest|all]         - Display CI execution logs',
      'feature-highlight',
      true
    );
    this.addOutput(
      '<span class="command-name">ci refresh</span>                   - Refresh CI status from GitHub',
      'feature-highlight',
      true
    );
    this.addOutput(
      '<span class="command-name">ci workflows</span>                 - List all configured workflows',
      'feature-highlight',
      true
    );
    this.addOutput('', 'info');
  ...
  async showCIStatus() {
    ...
    try {
      ...
      if (githubStatus) {
        ...
        this.addOutput(
          `${statusEmoji} Recent Workflow: ${githubStatus.recentRun.name}`,
          'feature-highlight'
        );
        ...
        this.addOutput(
          `🎯 Success Rate: ${githubStatus.summary.successRate}%`,
          'feature-highlight'
        );
        this.addOutput(
          `✅ Passed: ${githubStatus.summary.passed}/${githubStatus.summary.total} workflows`,
          'info'
        );
        this.addOutput(
          `❌ Failed: ${githubStatus.summary.failed}/${githubStatus.summary.total} workflows`,
          'info'
        );
          ...
          try {
            ...
            if (logResponse.ok) {
              ...
              if (logLines.length > 20) {
                this.addOutput('', 'info');
                this.addOutput(
                  `... [${logLines.length - 20} lines omitted] ...`,
                  'feature-highlight'
                );
                this.addOutput('', 'info');
