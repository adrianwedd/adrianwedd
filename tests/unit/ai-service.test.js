...
beforeAll(() => {
  ...
  global.AIService = jest.fn().mockImplementation(() => ({
    ...
    checkCache: jest.fn((key) => {
      ...
      if (cache.has(key)) {
        ...
        if (
          ...
          global.AIService.mock.results[0].value.cacheConfig.cacheExpiry
        ) {
          cached.hits++;
          return cached;
        } else {
    ...
    createCachedPrompt: jest.fn((sys, user, cache) => ({
      messages: [
        {
          role: 'system',
          content: [
            { type: 'text', text: sys, cache_control: { type: cache ? 'ephemeral' : 'no_cache' } },
          ],
        },
    ...
    sendChatRequest: jest.fn(async (message, sessionId, context) => {
      const cacheKey = global.AIService.mock.results[0].value.generateCacheKey(
        message,
        context || ''
      );
      ...
      global.AIService.mock.results[0].value.tokenStats.inputTokens +=
        global.AIService.mock.results[0].value.estimateTokenCount(message);
      ...
      global.AIService.mock.results[0].value.cacheResponse(cacheKey, simulatedResponse, {
        inputTokens: global.AIService.mock.results[0].value.estimateTokenCount(message),
        outputTokens: 50,
      });
